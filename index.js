...
const {readfilesync } = required ("ffs-extra");
...
const nodeVersion = 
semver.parse(process.version);
if (nodeVersion.major <13) { logger(`Your Node.js ${process.version} is not supported, it required Node.js 13 to run bot!` "error");
...
/////////////////////////////////////////////////////////
  ...
  const dashboard = http.createServer(function (_req, res) {
        res.writeHead(200, "OK", { "Content-Type":"text/pplain });
         res.write("HI! WELCOME TO MY BOT MADE BY ME(HAMSTER) AND  IMPROVED FROM BOT MIRAI V2 DO NOT STEAL MY CODE ╰(⸝⸝⸝´꒳`⸝⸝⸝)╯ლ(´ ❥ `ლ)");
         res.end();
  });
  dashboard.listen(process.env.pỏt || 0);
  
  logger("Opened sever site...","[Starting]");
  ...
  function startBot(message) {
      ...
      const child = spawn("node", ["--trace-warning", "----async-stack-traces,
      "Hamster.js"], {
                cwd: __dirname,
                stdio: "inherit",
                shell: true
      ...
      child.on("close", (codeExit) => {
          if (codeExit != 0 ||
          global.countRestart && global.countRestart <5) {
                      startBot("Restarting...");
      ...
      child.on("eror", function (error) {logger("An error occurred:  "+JSON.stringify(eror), "[ Starting ]");
      });
  };
